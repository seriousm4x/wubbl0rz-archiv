...
func CreateAuthFiles() error {
	...
	if err != nil {
		logger.Fatal.Printf("Unable to read client secret file: %v", err)
	}

	// If modifying the scope, delete your previously saved credentials
	config, err := google.ConfigFromJSON(b, scope)
	if err != nil {
		logger.Fatal.Printf("Unable to parse client secret file to config: %v", err)
	}
	...
	if err != nil {
		logger.Fatal.Printf("Unable to get path to cached credential file. %v", err)
	}
	tok, err := tokenFromFile(cacheFile)
	if err != nil {
		authURL := config.AuthCodeURL("state-token", oauth2.AccessTypeOffline)
		if launchWebServer {
			logger.Debug.Println("Trying to get token from web")
			tok, err = getTokenFromWeb(config, authURL)
		} else {
			logger.Debug.Println("Trying to get token from prompt")
...
func getTokenFromPrompt(config *oauth2.Config, authURL string) (*oauth2.Token, error) {
	var code string
	logger.Info.Printf("Go to the following link in your browser. After completing "+
		"the authorization flow, enter the authorization code on the command "+
		"line: \n%v\n", authURL)

	if _, err := fmt.Scan(&code); err != nil {
		logger.Fatal.Printf("Unable to read authorization code %v", err)
	}
...
func getTokenFromWeb(config *oauth2.Config, authURL string) (*oauth2.Token, error) {
	...
	if err != nil {
		logger.Error.Println("Unable to start a web server.")
		return nil, err
	}

	err = openURL(authURL)
	if err != nil {
		logger.Fatal.Fatalf("Unable to open authorization URL in web server: %v", err)
	} else {
		logger.Info.Println("Your browser has been opened to an authorization URL. This program will resume once authorization has been provided.")
		logger.Info.Println(authURL)
	}
...
func tokenCacheFile() (string, error) {
	return filepath.Join(url.QueryEscape("bearer_token.json")), nil
}
...
func saveToken(file string, token *oauth2.Token) {
	logger.Debug.Println("trying to save token")
	logger.Info.Printf("Saving credential file to: %s\n", file)
